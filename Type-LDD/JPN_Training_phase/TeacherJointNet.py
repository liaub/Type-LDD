...
class Teacher_Joint_Prediction(nn.Module):
    '''
    Teacher Joint prediction model
    ...
    def __init__(self,use_gpu,Data_Vector_Length,ModelSelect):
        super(Teacher_Joint_Prediction,self).__init__()
    ...
    def forward(self,datax,BASE_PATH):
        ...
        type_pred_T = self.prototypical_loss(input,BASE_PATH)
        loc_pred_T,loc_W = self.DriftPointNet(input, datax)
        # pre_loc_y=self.OnlyDriftPointNet(datax)
        # loc_loss = self.loc_loss_fun(loc_pred_T,locy)
        # loc_acc = self.cal_loc_acc(loc_pred_T,locy)
        # Combination of two Loss
        # loss = self.AutomaticWeightedLoss(class_loss, loc_loss)
        return type_pred_T,loc_pred_T,loc_W
...
class CNN(nn.Module):
    def __init__(self,Data_Vector_Length):
        ...
        self.Ks = [2,3,4]  ## kernel list，like [6,8,10]
...
class DriftPointNet(nn.Module):
    ...
    def forward(self,centroid,datax):
        ...
        return pre_loc_y,self.out.weight

class OnlyDriftPointNet(nn.Module):
    def __init__(self,use_gpu=True,Data_Vector_Length=100):
        super(OnlyDriftPointNet, self).__init__()
        self.Data_Vector_Length=Data_Vector_Length
        self.fc1 = nn.Linear(self.Data_Vector_Length, 800)
